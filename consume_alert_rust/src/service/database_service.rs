...
pub trait DBService {
    ...
    //async fn get_classification_consumption_type(&self, index_name: &str) -> Result<HashMap<String, ConsumingIndexProdType>, anyhow::Error>;
    async fn get_consume_detail_specific_period(
        &self,
        start_date: NaiveDate,
        end_date: NaiveDate,
    ) -> Result<ConsumeGraphInfo, anyhow::Error>;
    // async fn get_consume_detail_specific_period__deprecated(
    //     &self,
    //     start_date: NaiveDate,
    //     end_date: NaiveDate,
    // ) -> Result<(f64, Vec<ConsumeIndexProd>), anyhow::Error>;
    ...
    async fn get_recent_mealtime_data_from_elastic(
        &self,
        query_size: i32,
    ) -> Result<Vec<MealCheckIndex>, anyhow::Error>;

    async fn post_model_to_es<T: Serialize + Send>(
        &self,
        index_name: &str,
        model: T,
    ) -> Result<(), anyhow::Error>;
    ...
    async fn get_recent_consume_info_order_by(
        ...
        order_by_info: &str,
        size: i64,
    ) -> Result<Vec<(String, ConsumeIndexProdNew)>, anyhow::Error>;
...
impl DBService for DBServicePub {
        ...
        if let Some(hit_array) = hits.as_array() {
            for hit in hit_array {
                let doc_id = hit
                    ...
                    .ok_or_else(|| {
                        anyhow!(
                            "[Error][get_recent_consume_info_order_by()] Missing 'doc_id' field"
                        )
                    })?
                    .to_string();
                ...
                let consume_info: ConsumeIndexProdNew = hit
                    .get("_source")
                    .ok_or_else(|| {
                        anyhow!(
                            "[Error][get_recent_consume_info_order_by()] Missing '_source' field"
                        )
                    })
                    .and_then(|source| {
    ...
    async fn get_consume_detail_specific_period(
        ...
        let total_cost = match &response_body["aggregations"]["total_prodt_money"]["value"].as_f64()
        {
            ...
            None => {
                return Err(anyhow!(
                    "[Error][get_consume_detail_specific_period()] 'total_cost' error"
                ))
            }
        };
        ...
        let consume_grapth_info: ConsumeGraphInfo =
            ConsumeGraphInfo::new(total_cost, consume_list, start_date, end_date);
    ...
    //         None => {
    //             return Err(anyhow!(
    //                 "[Error][total_cost_detail_specific_period()] 'total_cost' error"
    //             ))
    //         }
    //     };
    ...
    //     let mut results: Vec<ConsumeIndexProd> = hits
    //         .as_array()
    ...
    //                 .ok_or_else(|| {
    //                     anyhow!(
    //                         "[Error][total_cost_detail_specific_period()] Missing '_source' field"
    //                     )
    //                 })
    //                 .and_then(|source| serde_json::from_value(source.clone()).map_err(Into::into))
    //         })
    ...
    //                 return Err(anyhow!(
    //                     "[Error][get_classification_consume_detail()] {:?}",
    //                     e
    //                 ));
    //             }
    //             Err(e) => {
    //                 /* Error in TASK */
    //                 return Err(anyhow!(
    //                     "[Error][get_consume_detail_specific_period()] {:?}",
    //                     e
    //                 ));
    //             }
    //         }
    ...
    async fn get_recent_mealtime_data_from_elastic(
        ...
        let response_body = es_client
            .get_search_query(&es_query, "meal_check_index")
            .await?;
        let hits = &response_body["hits"]["hits"];
...
#[doc = ""]
async fn get_classification_consume_detail_v1(
    consume_details: &mut Vec<ConsumeIndexProd>,
) -> Result<(), anyhow::Error> {
    let es_client = get_elastic_conn()?;
...
// ///
// ///
// async fn get_classification_consume_detail(
//     consume_details: Vec<ConsumeIndexProd>,
// ) -> Result<Vec<ConsumeIndexProd>, anyhow::Error> {
...
//             let top_score_consume_type = match score_pair
//                 .iter()
