...
pub trait GraphApiService {
    ...
    async fn post_api<T: Serialize + Send>(&self, uri: &str, to_python_graph: T) -> Result<String, anyhow::Error>;
...
impl GraphApiService for GraphApiServicePub {
    ...
    async fn post_api<T: Serialize + Send>(&self, uri: &str, to_python_graph: T) -> Result<String, anyhow::Error> {
        ...
        let res = self.client.post(&post_uri)
        ...
        if res.status().is_success() {
            ...
            Err(anyhow!("[Error][cpost_api()] Request for '{}' failed.", &post_uri))
        }
    }
    ...
    async fn call_python_matplot_consume_detail(&self, comparison_info: &Vec<ToPythonGraphLine>) -> Result<String, anyhow::Error> {
        ...
        let resp_body = self.post_api("/api/consume_detail", comparison_info).await?;
        Ok(resp_body)
    ...
    async fn get_consume_type_graph(&self ,total_cost: f64, start_dt: NaiveDate, end_dt: NaiveDate, consume_list: &Vec<ConsumeInfo>) -> Result<(Vec<ConsumeTypeInfo>, String), anyhow::Error> {
        ...
        let png_path = self.call_python_matplot_consume_type(&consume_type_list, start_dt, end_dt, total_cost).await?;
