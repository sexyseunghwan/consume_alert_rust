...
pub trait CommandService {
    ...
    /* Service */
    async fn process_by_consume_type(
        &self,
        split_args_vec: &Vec<String>,
    ) -> Result<ConsumeIndexProdNew, anyhow::Error>;
    fn get_consume_prodt_name(
        &self,
        consume_time_name_vec: &Vec<String>,
        idx: usize,
    ) -> Result<String, anyhow::Error>;
...
impl CommandService for CommandServicePub {
    ...
    async fn process_by_consume_type(
        ...
        if consume_type.contains("nh") {
            ...
            let prodt_type: String = self.get_consume_type_judgement(consume_name).await?;

            res_struct = ConsumeIndexProdNew::new(
                consume_time,
                cur_timestamp,
                consume_name.clone(),
                consume_price,
        ...
        es_client
            .post_query_struct(&res_struct, CONSUME_DETAIL)
            .await?;
    ...
    async fn get_consume_type_judgement(&self, prodt_name: &str) -> Result<String, anyhow::Error> {
        ...
        if results.len() == 0 {
            ...
            let mut manager: ScoreManager<ConsumingIndexProdType> =
                ScoreManager::<ConsumingIndexProdType>::new();
            ...
            let score_data_keyword: ScoredData<ConsumingIndexProdType> = match manager.pop_lowest()
            {
