...
impl RedisRepositoryImpl {
    ...
    pub async fn new() -> anyhow::Result<Self> {
        ...
        let conn: RedisConnectionType = if redis_url.contains(',') {
            ...
            let cluster_client: ClusterClient = ClusterClient::new(nodes)
                .map_err(|e| anyhow!("[RedisRepositoryImpl::new] Failed to create Redis cluster client: {:?}", e))?;

            let cluster_conn: ClusterConnection = cluster_client
                .get_async_connection()
                .await
                .map_err(|e| anyhow!("[RedisRepositoryImpl::new] Failed to connect to Redis cluster: {:?}", e))?;

            info!("[RedisRepositoryImpl::new] Connected to Redis cluster with {} nodes", node_count);
            RedisConnectionType::Cluster(cluster_conn)
        } else {
            // Single node mode
            let client = redisClient::open(redis_url.as_str())
                .map_err(|e| anyhow!("[RedisRepositoryImpl::new] Failed to create Redis client: {:?}", e))?;

            let single_conn: MultiplexedConnection = client
                .get_multiplexed_async_connection()
                .await
                .map_err(|e| anyhow!("[RedisRepositoryImpl::new] Failed to connect to Redis: {:?}", e))?;
        ...
        match &self.conn {
            RedisConnectionType::Single(conn) => {
                ...
                let result: Option<String> = conn
                    .get(key)
                    .await
                    .map_err(|e: RedisError| anyhow!("[RedisRepositoryImpl::get] Failed to get key '{}': {:?}", key, e))?;
                Ok(result)
            }
    ...
    async fn set(&self, key: &str, value: &str) -> anyhow::Result<()> {
        match &self.conn {
            RedisConnectionType::Single(conn) => {
                ...
                conn.set(key, value)
                    .await
                    .map_err(|e: RedisError| anyhow!("[RedisRepositoryImpl::set] Failed to set key '{}': {:?}", key, e))?;
                Ok(())
            }
            RedisConnectionType::Cluster(conn) => {
                let mut conn = conn.clone();
    ...
    async fn del(&self, key: &str) -> anyhow::Result<()> {
        match &self.conn {
            RedisConnectionType::Single(conn) => {
                ...
                conn.del(key)
                    .await
                    .map_err(|e: RedisError| anyhow!("[RedisRepositoryImpl::del] Failed to delete key '{}': {:?}", key, e))?;
                Ok(())
            }
            RedisConnectionType::Cluster(conn) => {
                let mut conn = conn.clone();
        ...
        match &self.conn {
            RedisConnectionType::Single(conn) => {
                ...
                let exists: bool = conn
                    .exists(key)
                    .await
                    .map_err(|e: RedisError| anyhow!("[RedisRepositoryImpl::exists] Failed to check key '{}': {:?}", key, e))?;
                Ok(exists)
            }
            RedisConnectionType::Cluster(conn) => {
                let mut conn = conn.clone();
