...
use crate::models::{
    asset_resp::*, cash_asset::*, crypto_resp::*, currency_exchange_rate_snapshot::*,
    deposit_asset::*, earned_detail::*, per_datetime::*, saving_asset::*, stock_resp::*,
};
    ...
    > MainController<G, E, M, T, P, KP, R, C>
{
    ...
    pub(super) async fn command_show_all_asset(
        ...
        match args.len() {
            1 => {
                let currency_list: Vec<&str> = vec!["KRW", "USD"];
                ...
                let mut total_krw: Decimal = Decimal::ZERO;
                let mut total_usd: Decimal = Decimal::ZERO;
                let mut asset_resp_map: HashMap<String, Vec<AssetResp>> = HashMap::new();
                ...
                for currency_code in currency_list {
                    ...
                    let deposits: Vec<DepositAsset> = self
                        ...
                        .await
                        .inspect_err(|e| {
                            error!("[command_show_all_asset] Error at `deposits` : {:#}", e)
                        })?;

                    for deposit in &deposits {
                        let amount: Decimal = *deposit.deposit_amount();

                        if currency_code == "KRW" {
                            let krw_to_usd_amount: Decimal = amount * krw_to_usd;

                            let asset_resp: AssetResp = AssetResp::new(
                                String::from("Deposit"),
                                deposit.deposit_name().to_string(),
                                amount,
                                krw_to_usd_amount,
                            );

                            asset_resp_map
                                .entry(String::from("Deposit"))
                                .or_insert(Vec::new())
                                .push(asset_resp);

                            total_krw += amount;
                        } else {
                            let usd_to_krw_amount: Decimal = amount * usd_to_krw;
                    ...
                    let savings: Vec<SavingAsset> = self
                        ...
                        .await
                        .inspect_err(|e| {
                            error!("[command_show_all_asset] Error at `savings` : {:#}", e)
                        })?;

                    for saving in &savings {
                        let amount: Decimal = *saving.accum_saving_amount();
                        ...
                        if currency_code == "KRW" {
                            ...
                            let asset_resp: AssetResp = AssetResp::new(
                                String::from("Saving"),
                                saving.saving_name().to_string(),
                                amount,
                                krw_to_usd_amount,
                            );

                            asset_resp_map
                                .entry(String::from("Saving"))
                                .or_insert(Vec::new())
                                .push(asset_resp);
                    ...
                    let stocks: Vec<StockResp> = self
                        ...
                        .await
                        .inspect_err(|e| {
                            error!("[command_show_all_asset] Error at `stocks`: {:#}", e);
                        })?;

                    for stock in &stocks {
                        let amount: Decimal = *stock.stock_total_price();
                        ...
                        if currency_code == "KRW" {
                            ...
                            let asset_resp: AssetResp = AssetResp::new(
                                String::from("Stock"),
                                stock.stock_name().to_string(),
                                amount,
                                krw_to_usd_amount,
                            );

                            asset_resp_map
                                .entry(String::from("Stock"))
                                .or_insert(Vec::new())
                                .push(asset_resp);
                    ...
                    let cryptos: Vec<CryptoResp> = self
                        ...
                        .await
                        .inspect_err(|e| {
                            error!("[command_show_all_asset] Error at `cryptos` {:#}", e);
                        })?;

                    for crypto in &cryptos {
                        let amount: Decimal = *crypto.crypto_total_price();
                        ...
                        if currency_code == "KRW" {
                            ...
                            let asset_resp: AssetResp = AssetResp::new(
                                String::from("Crypto"),
                                crypto.crypto_name().to_string(),
                                amount,
                                krw_to_usd_amount,
                            );

                            asset_resp_map
                                .entry(String::from("Crypto"))
                                .or_insert(Vec::new())
                                .push(asset_resp);
                    ...
                    let cashes: Vec<CashAsset> = self
                        ...
                        .await
                        .inspect_err(|e| {
                            error!("[command_show_all_asset] Error at `cashes`: {:#}", e);
                        })?;

                    for cash in &cashes {
                        let amount: Decimal = *cash.cash();
                        ...
                        if currency_code == "KRW" {
                            ...
                            let asset_resp: AssetResp = AssetResp::new(
                                String::from("Cash"),
                                cash.cash_name().to_string(),
                                amount,
                                krw_to_usd_amount,
                            );

                            asset_resp_map
                                .entry(String::from("Cash"))
                                .or_insert(Vec::new())
                                .push(asset_resp);
